...
// Note: Ensure GSAP is loaded in index.html import map: "gsap": "https://esm.sh/gsap@3.12.5"

const DEFAULT_PARTICLE_COUNT = 8;
const DEFAULT_SPOTLIGHT_RADIUS = 300;
const DEFAULT_GLOW_COLOR = '124, 58, 237'; // Violet-600 RGB
...
const GlobalSpotlight = ({
  ...
  useEffect(() => {
    ...
    const handleMouseLeave = () => {
      ...
      // Fixed: Check if gridRef.current exists before accessing querySelectorAll
      if (gridRef.current) {
        const cards = gridRef.current.querySelectorAll('.magic-bento-card');
        if (cards) {
            cards.forEach((card: any) => {
    ...
    <>
      ...
      <BentoCardGrid gridRef={gridRef}>
        {items.map((card, index) => {
          ...
          // Use CSS variables for color if needed, currently using class defaults
          const cardProps = {
